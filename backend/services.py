...
async def create_user(user: _schemas.UserCreate, db: _orm.Session):
    hashed_password = _hash.bcrypt.hash(user.password)  # Utiliser 'user.password' et non 'user.hashed_password'
...
async def create_token(user: _models.User):
    user_obj = _schemas.UserCreate.model_validate(user)  # Utiliser UserCreate au lieu de User

    token = _jwt.encode(user_obj.model_dump(),JWT_SECRET)
    return dict(access_token=token,token_type ="bearer" )

async def get_current_user(db:_orm.Session  = _fastapi.Depends(get_db), token:str =_fastapi.Depends(_security.OAuth2PasswordBearer(tokenUrl="/api/token"))):
    try:
...
async def update_lead(lead_id: int, lead: _schemas.LeadCreate, user: _schemas.User, db: _orm.Session):
    ...
    lead_db.role = lead.role
    
    db.commit()
    db.refresh(lead_db)

    return _schemas.Lead.model_validate(lead_db)
""" 
import fastapi as _fastapi
import fastapi.security as _security
import jwt as _jwt
